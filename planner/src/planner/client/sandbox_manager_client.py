...
class LocalSandboxManager(SandboxManager):
    """
    Local sandbox manager - uses local machine instead of remote ECS
    """
    _instance = None
    ...
    def __init__(self):
        if self._initialized:
            return
        self.logger = logging.getLogger(self.__class__.__name__)
        self.local_endpoint = get_settings().sandbox.local_tool_server_endpoint
        self._initialized = True

    def get_tool_server_endpoint(self, sandbox_id: str = None) -> str | None:
        """
        Returns local tool server endpoint (ignores sandbox_id)
        """
        self.logger.info("Using local tool server: %s", self.local_endpoint)
