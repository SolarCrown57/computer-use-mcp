...
from common.constants import COMPUTER_USE, MODEL_GEMINI_FLASH, MODEL_GEMINI_FLASH_NOTHINKING

MODEL_TOOL_CALL_ADAPTER_MAP = {
    COMPUTER_USE: {
        MODEL_GEMINI_FLASH: DoubaoUITarsToComputerUseMCPAdaptor,
        MODEL_GEMINI_FLASH_NOTHINKING: DoubaoUITarsToComputerUseMCPAdaptor,
    }
}
    ...
    async def run_task(self) -> AsyncGenerator[str, Any]:
        ...
        try:
            for step in range(self.max_actions):
                ...
                self.logger.debug("Raw model response: %s", response[:500] if response else "None")

                # Extract thinking content if present (for Gemini and other models with thinking)
                thinking_content = None
                if response and "<thinking>" in response and "</thinking>" in response:
                    import re
                ...
                # Handle case where action is None (parsing failed)
                if action is None:
                    self.logger.warning("Failed to parse action from response, raw response: %s", response[:300] if response else "None")
                    yield self._format_sse({
                        "summary": summary or "模型响应解析失败，请重试",
                        "thinking": thinking_content,
                        "task_id": self.task_id,
                        "error": "无法解析模型响应中的Action"
                    })
                    break
