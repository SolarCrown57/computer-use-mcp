...
import { LocalDesktop } from "./local-desktop";
  ...
  return (
    <div className="h-full flex-1 w-full border-0 shadow-sm bg-white overflow-hidden rounded-lg flex flex-col">
      ...
      <div className="flex-1 w-full h-full bg-white relative ">
        {isLocalMode && <LocalDesktop />}
        {!isLocalMode && sandbox?.OsType === OSType.LINUX && <LinuxDesktop />}
        {!isLocalMode && sandbox?.OsType === OSType.WINDOWS && <WindowsDesktop />}
      </div>
    </div>
  );
};
